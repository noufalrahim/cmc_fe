...
export const AnesthesiaSignOut = async (token: string | null, patientId: string, fetchPatientsData: () => void, by: string) => {
    if (!token || !patientId) {
    ...
    try {
        const timelineResponseData = await postTimeline(token, {
            patientId: patientId,
            action: ActionGenerator(Status.Ongoing.AnesthesiaSignedOut),
            status: Status.Ongoing.AnesthesiaSignedOut,
            by: by,
            details: 'Anesthesia signed Out',
        });
